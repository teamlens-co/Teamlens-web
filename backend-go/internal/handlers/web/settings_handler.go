...
func (h *SettingsHandler) GetPublicEnv(w http.ResponseWriter, r *http.Request) {
	threshold := 180
	if auth := middleware.GetAuthContext(r.Context()); auth != nil && auth.OrganizationID != "" {
	...
	env := map[string]interface{}{
		"googlePlacesApiKey":           "",
		"appVersion":                   "1.0.0",
		"productivityThresholdMinutes": threshold,
...
func (h *SettingsHandler) UpdateSettings(w http.ResponseWriter, r *http.Request) {
	...
	var input struct {
		ProductivityThresholdMinutes int `json:"productivityThresholdMinutes"`
	}
	if err := json.NewDecoder(r.Body).Decode(&input); err != nil {
		middleware.Error(w, http.StatusBadRequest, "Invalid request body")
		return
	}
	if input.ProductivityThresholdMinutes <= 0 || input.ProductivityThresholdMinutes > 1440 {
		middleware.Error(w, http.StatusBadRequest, "productivityThresholdMinutes must be between 1 and 1440")
		return
	}

	_, err := h.pool.Exec(r.Context(),
		`UPDATE organizations
		 SET productivity_threshold_minutes = $1, updated_at = NOW()
		 WHERE id = $2`,
		input.ProductivityThresholdMinutes,
		auth.OrganizationID,
	)
	if err != nil {
		slog.Error("Failed to update settings", "error", err)
		middleware.Error(w, http.StatusInternalServerError, "Unable to save settings")
		return
	}

	middleware.Success(w, http.StatusOK, map[string]int{"productivityThresholdMinutes": input.ProductivityThresholdMinutes})
}
