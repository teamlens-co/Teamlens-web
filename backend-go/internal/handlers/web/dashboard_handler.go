...
func (h *DashboardHandler) GetAttendance(w http.ResponseWriter, r *http.Request) {
	auth := middleware.GetAuthContext(r.Context())
	if auth == nil {
		middleware.Error(w, http.StatusUnauthorized, "Unauthorized")
		return
	...
	result, err := h.service.GetAttendance(r.Context(), auth.OrganizationID, auth.UserID, auth.Role, requestedUserID, start, end)
	if err != nil {
		slog.Error("Attendance overview failed", "error", err)
		middleware.Error(w, http.StatusInternalServerError, "Unable to fetch attendance")
		return
	}
